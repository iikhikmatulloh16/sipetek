                    ...
                    <div class="col-sm-4">
                        <div class="text-center mt-sm-0 mt-3 text-sm-end">
                            ...
                            <form action="admin/pengaduans/{{ $pengaduan->slug }}" method="POST" class="d-inline">
                                @method('put')
                                @csrf
                                <button type="submit" class="btn btn-light"><i class="bx bx-loader-circle" ></i> Konfirmasi</button>
                                {{-- <a href="admin/pengaduans/{{ $pengaduan->slug }}/edit" type="button" class="btn btn-light"><i class="mdi mdi-check me-1"></i> Proses</a> --}}
                            </form>
                            <a href="{{ route('tanggapans.show', $pengaduan->id) }}" type="button" class="btn btn-light disabled"><i class="mdi mdi-forum-outline"></i> Tanggapi</a>
                            @elseif($pengaduan->status == 'Proses') 
                                <form action="admin/pengaduans/{{ $pengaduan->slug }}" method="POST" class="d-inline">
                                    @method('put')
                                    @csrf
                                    <button type="submit" class="btn btn-light disabled"><i class="bx bx-loader-circle"></i> Konfirmasi</button>
                                    {{-- <a href="admin/pengaduans/{{ $pengaduan->slug }}/edit" type="button" class="btn btn-light"><i class="mdi mdi-check me-1"></i> Proses</a> --}}
                                </form>
                                <a href="{{ route('tanggapans.show', $pengaduan->id) }}" type="button" class="btn btn-light"><i class="mdi mdi-forum-outline"></i> Tanggapi</a>
                            @else
                                <a href="admin/pengaduans/{{ $pengaduan->slug }}/edit" type="button" class="btn btn-success disabled"><i class="mdi mdi-check-all" ></i> Selesai</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row justify-content-center">
                            <div class="col-md-12">
                                ...
                                {{-- @if ($pengaduan->tanggapan->image)
                                    <img src="{{ asset('storage/' . $pengaduan->tanggapan->image) }}" alt="" class="img-thumbnail" width="100%" />
                                @else
                                    <img src="{{ asset('storage/tanggapan-images/' . '_no-image.png') }}" alt="" class="img-thumbnail" width="100%" />
                                @endif --}}

                                {{-- <img src="img/blog1.jpg" alt="" class="img-thumbnail" width="100%" /> --}}
                            </div>
                        </div>
                    </div>
                    <!-- Tanggapan End -->
